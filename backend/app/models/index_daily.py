...
class IndexDaily(db.Model):
    ...
    ts_code = db.Column(db.String(16), nullable=False, comment='TS代码')
    trade_date = db.Column(db.Date, nullable=False, comment='交易日期')
    close = db.Column(db.Numeric(12, 4), comment='收盘点位')
    open = db.Column(db.Numeric(12, 4), comment='开盘点位')
    high = db.Column(db.Numeric(12, 4), comment='最高点位')
    low = db.Column(db.Numeric(12, 4), comment='最低点位')
    pre_close = db.Column(db.Numeric(12, 4), comment='昨收点位')
    change = db.Column(db.Numeric(12, 4), comment='涨跌点位')
    pct_chg = db.Column(db.Numeric(8, 4), comment='涨跌幅（%）')
    vol = db.Column(db.Numeric(20, 4), comment='成交量')
    amount = db.Column(db.Numeric(20, 4), comment='成交额（千元）')
